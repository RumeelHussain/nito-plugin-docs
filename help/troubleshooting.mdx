...
`private` relies on a Codex **hook** to keep the turn out of your host's view. Until you trust that hook, the plugin **fails closed**: it blocks the private request rather than risk sending it through unprotected. That is the privacy guarantee working, not a failure.
...
<Steps>
  ...
  <Step title="Retry in a fresh turn">
    Run your `$nito:private` command again. It now goes through, and the host does not see the turn.
  </Step>
</Steps>

Only `private` (and session-style privacy) needs this hook. `ask`, `status`, and `models` work without it. A successful private turn reports `Session mode: Incognito` and is handled by Nito rather than your host.
