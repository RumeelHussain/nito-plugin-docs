...
This is a known compatibility issue between a recent Codex release and the gateway: Codex sends a verbosity setting the gateway does not yet accept, and the request is rejected. It is being tracked as a gateway fix, and it is **not** a problem with your account or sign-in.

Until it is resolved:

- Prefer the in-chat `$nito:` commands (`$nito:ask`, `$nito:private`, and so on) inside your normal Codex session, which are not affected.
- Retry the Nito-routed session after updating with `nito update`, which pulls the fix once it ships.
