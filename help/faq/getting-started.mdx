...
Inside a running Claude Code or Codex session, not your terminal shell. `/nito:ask` and `$nito:ask` are commands the host understands; your shell does not. The one exception is the `nito` [CLI](/help/faq/using-the-cli), which does run in the terminal.
