...
`nito claude` and `nito codex` open a host session whose model is **entirely a Nito model**, from your terminal. They are the CLI form of the in-chat [`session`](/commands/session) command: the host runs normally, but every model turn goes to Nito instead of the host's own provider.
...
```text
nito claude --model <logical-model-id> [-- <claude args>] [flags]
```
...
```text
nito codex --model <logical-model-id> [-- <codex args>] [flags]
```
...
Standard and Confidential models can power a session. Encrypted models are not part of this release. See [Models](/models) for the catalog and [Privacy Tiers](/privacy/tiers) for what each tier reaches.

## Relationship to the In-Chat Command

`nito claude` / `nito codex` and `/nito:session` / `$nito:session` open the same kind of isolated Nito-routed session. Use the plugin command when you are already in a chat; use the CLI launcher when you want to start one from a script, an alias, or a fresh terminal.
