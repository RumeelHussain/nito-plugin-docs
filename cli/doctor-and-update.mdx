...
`doctor` checks your Nito installation, your stored credentials, and each host integration, then reports what passed and what needs attention. Run it first whenever a command misbehaves.
...
| `--repair` | Repair local Nito state and remove legacy host integrations. |
...
`update` upgrades the Nito binary through your package manager and refreshes the managed host integrations so the plugin bundle and hooks match the new version.
...
For host-specific reconnection inside a chat, see [When Tools Do Not Load in a Chat](/commands/status-and-account).
