...
These tools are a different transport, not a different set of rules. The privacy tier still follows the model, premium and beta model policy is enforced on `nito.chat_complete`, and rate limits are returned as normal tool errors with canonical `rate_limited` codes when you go over. Prompts, tool arguments, and completions are not persisted or logged by default.

## Running the Local Bridge

If your agent needs a local MCP endpoint, the CLI can run one:

```bash
nito agent mcp
```

This starts the optional local MCP bridge, which mirrors the same documented tools. For the full protocol details, scopes, and paging behavior, see the platform [MCP reference](/api-reference).
