...
There is no privacy flag on your command. The level is part of the model's name. Every model carries its own privacy tier, and you get the tier of the model you choose. Browse the catalog with the `models` command, then set your model with `model` or `--model`. The tier comes along with it.
