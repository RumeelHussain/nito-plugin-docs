...
These are two separate catalog entries. You do not invent the marker or staple it onto any model; it is part of the name of a model that offers the Confidential level, and you select that entry the same way you select any model. Choosing the `:confidential` entry runs your call in a sealed environment you can verify. The [Model List](/models/all-models) shows which models have a Confidential entry.

<Note>
The catalog also has an **Encrypted** level, but Encrypted is **not yet available through the plugin**. Today the plugin reaches up to Confidential. See [Encrypted](/privacy/tiers/encrypted).
</Note>

## The Level Is Part of the Model

This is the key idea: a model's privacy level is a property of the model you choose, not a separate switch you flip. The plain entry and the `:confidential` entry are different models at different levels. Because the level travels with the model, it cannot drift, and it never silently downgrades: if a model cannot serve a call at its level, the call fails with a clear message rather than dropping to something weaker. See [Level Follows the Model](/privacy/tiers).
...
- **The level follows the model.** Pick the entry whose level matches how sensitive the call is. See [The Four Levels](/privacy/tiers).
