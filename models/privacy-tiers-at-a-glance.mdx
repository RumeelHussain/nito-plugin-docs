...
In the plugin you do not set privacy once for your account. You choose it per prompt by choosing a model, because [the level follows the model](/privacy/tiers). The same setup can send a throwaway question on a light model and a sensitive one on a Confidential model, with no account-wide switch to remember.
...
- **Confidential** is the first level that gives you a **proof**: Private holds the provider to a promise, while Confidential gives you an attestation you can verify yourself. See [Verify It Yourself](/privacy/tee-attestation).
