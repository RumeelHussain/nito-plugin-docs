...
In the plugin you do not set privacy once for your account. You choose it per prompt by choosing a model, because [the level follows the model](/privacy/levels). The same setup can send a throwaway question on a light model and a sensitive one on a Confidential model, with no account-wide switch to remember.
