---
title: "Features"
description: "What the models can do through the plugin: reasoning, web search and fetch, documents, and images. Most turn on with a flag on a command you already use."
---
...
When you open a full [`session`](/commands/session), your harness's normal behaviour keeps working, now running through Nito. Streaming replies and its own tool use are harness-native rather than plugin flags, so they are covered where the session is.

## From the Terminal

Every capability here also works from the `nito` command line tool. See [Ask and Fusion from the Terminal](/cli/ask-and-fusion-from-terminal).
