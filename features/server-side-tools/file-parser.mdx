...
When you attach a document, Nito extracts its text so the model can read it. You do not paste the contents by hand and you do not need a special model: Nito parses the document into text before the model sees it, so this works with any model you pick.
