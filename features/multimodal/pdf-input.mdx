...
When you need a model to reason over a contract, a slide deck, or a spreadsheet, you do not paste the text by hand. You attach the document with `--file` and Nito reads it for you. This works with any model, because Nito extracts the document's text before the model sees it. For the general mechanism, see [File parser](/features/server-side-tools/file-parser).
