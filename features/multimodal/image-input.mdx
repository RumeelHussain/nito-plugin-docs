---
title: "Image Input"
description: "Attach a picture with --file and let an image-capable model read it: describe a screenshot, transcribe a receipt, or read a chart from your terminal or editor."
---
...
<Tabs>
  <Tab title="Claude Code">
    ```text
    /nito:ask --model <image-capable-model> --file chart.png What does this chart show?
    ```
  </Tab>
  <Tab title="Codex">
    ```text
...
<Tabs>
  <Tab title="Claude Code">
    ```text
    /nito:ask --model <image-capable-model> --file before.png --file after.png What changed between these two screenshots?
    ```
  </Tab>
  <Tab title="Codex">
    ```text
...
## Images Require an Image-Capable Model
...
```text
model "<id>" cannot accept image attachments; choose an image-capable model or retry without the image
```
...
Real image-capable model IDs (for example an `anthropic/claude-...` vision model) appear in the catalog, but prefer describing what you need ("an image-capable model") and let the `models` list confirm the current options. See [Choosing a model](/commands/choosing-a-model).
