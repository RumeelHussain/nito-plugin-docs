...
description: "Ask a Nito model one question inside your current chat, then continue that exchange, without changing your host model."
---

`ask` sends one prompt to a Nito model and returns the answer inside your current chat. `continue` follows up on that same exchange. Both make **at most one** Nito call and leave your host model untouched.

<Note>
`ask` is a normal turn in your host chat, so **your host model can see the ask prompt and answer**. It is a "second opinion," not a private one. When a prompt must stay out of the host's request, use [`private`](/commands/private) instead.
</Note>
...
The result is labeled with the environment, model, privacy tier, and context scope, for example `Nito · Hosted · MODEL-ID · private · Prompt only`.
...
By default `ask` sends **only your prompt** to Nito, and the exchange stays in your host chat where the host model can see it. Turning [Context](/commands/context-and-mode) on lets `ask` include a short host-generated summary of the task; nothing is sent that you did not enable.
