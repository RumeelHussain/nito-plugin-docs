...
description: "Ask one question through Nito inside your current chat, then continue that exchange, without changing the model your harness runs."
---

`ask` sends one prompt to a model through Nito and returns the answer inside your current chat. `continue` follows up on that same exchange. Both make **at most one** Nito call and leave your harness model untouched.
...
The result is labeled with the environment, model, privacy level, and context scope, for example `Nito · Hosted · MODEL-ID · private · Prompt only`.
