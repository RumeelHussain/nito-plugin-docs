...
`private` is one of the plugin commands. It has two behaviors depending on whether you give it a prompt:

- **With a prompt**, it runs a one-shot exchange that your harness model's upstream **never sees**. The prompt and the Nito answer are kept out of the harness's request, and the answer is shown back to you as a blocked turn.
...
Anonymous, Private, and Confidential models are supported. Encrypted models are not included in this release and are refused with a clear message.
