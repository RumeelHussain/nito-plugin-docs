...
Free accounts get **10 requests per UTC calendar month**. The allowance is shared, so a request from Claude Code and a request from Codex draw from the same pool.
...
A request is a call that reaches a model. An [`ask`](/commands/ask-and-continue) and a [`private`](/commands/private) question each count once.
